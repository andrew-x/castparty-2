...
- `src/lib/types.ts` — Shared domain types: `CustomForm`, `CustomFormFieldType` (TEXT, TEXTAREA, SELECT, CHECKBOX_GROUP, TOGGLE, IMAGE, DOCUMENT, VIDEO), `CustomFormResponse` (fieldId + textValue/booleanValue/optionValues/fileValues), `SystemFieldConfig` (phone/location/headshots/resume/video/links/unionStatus/representation), `SystemFieldVisibility`, `RestrictedFieldVisibility`, `DEFAULT_SYSTEM_FIELD_CONFIG`, `SYSTEM_FIELD_LABELS`, `SYSTEM_FIELD_ALLOWED_VISIBILITIES`, `UNION_OPTIONS`, `Representation` (name/email/phone), `EmailTemplate`, `EmailTemplates` (submissionReceived/rejected/selected)
...
|-------|-------|-------------|
| `user` | `User` | `id`, `name`, `firstName`, `lastName`, `email`, `role`, `banned` |
...
| `Production` | Organization | `organizationId`, `name`, `slug` (unique per org), `description` (text), `status` enum (`"open"/"closed"/"archive"`), `location`, `banner` (text, nullable — R2 URL for production banner image), `submissionFormFields` (JSONB `CustomForm[]`), `feedbackFormFields` (JSONB `CustomForm[]`), `systemFieldConfig` (JSONB `SystemFieldConfig` — visibility of: phone, location, headshots, resume, video, links, unionStatus, representation; see `src/lib/types.ts`), `rejectReasons` (JSONB string[]), `emailTemplates` (JSONB `EmailTemplates` or null) |
| `Role` | Production | `productionId`, `name`, `slug` (unique per production), `status` enum (`"open"/"closed"/"archive"`), `description`, `referencePhotos` (JSONB string[] — R2 URLs of reference images shown on the public submission page) — no form fields, system field config, or reject reasons; all config lives on the parent `Production` |
...
| `Submission` | Role + Candidate | `productionId`, `roleId`, `candidateId`, `stageId` (FK → PipelineStage, `onDelete: "restrict"`), `rejectionReason` (nullable), `sortOrder` (text — fractional index for kanban ordering), `answers` (JSONB `CustomFormResponse[]`), `links` (text[]), `videoUrl` (text, nullable), `unionStatus` (text[], AEA/SAG-AFTRA etc.), `representation` (JSONB `Representation|null`), `resumeText` (nullable — extracted PDF text) |
| `PipelineUpdate` | Submission | `submissionId`, `fromStage`, `toStage`, `changeByUserId` — full audit trail of stage transitions |
| `File` | Submission or Candidate | `submissionId` (nullable), `candidateId` (nullable), `formFieldId` (nullable — links to the custom form field that collected this file), `type` enum (`HEADSHOT`/`RESUME`/`VIDEO`/`CUSTOM_FIELD`), `url`, `key`, `path`, `filename`, `contentType`, `size`, `order` |
...
`SystemFieldConfig` controls whether each system submission field (phone, location, headshots, resume, video, links, unionStatus, representation) is `"hidden"`, `"optional"`, or `"required"`. Note: `links`, `unionStatus`, and `representation` only support `"hidden"/"optional"` — not `"required"`. Stored as JSONB on `Production`; defaults vary per field (most optional, video/unionStatus/representation hidden). All roles in a production use the same config.
...
|---|----------|-------|--------|
| 1 | ~~`src/actions/candidates/get-candidate.ts`~~ | ~~`getCandidate` fetches the candidate row before checking `candidate.organizationId !== orgId`~~ **Fixed (2026-04-05):** The query now filters at the DB level with `and(eq(c.id, candidateId), eq(c.organizationId, orgId))` — no data is loaded then discarded. | ~~Performance~~ |
...
| 3 | ~~`src/actions/submissions/create-submission.ts`~~ | ~~Required-field validation for `TOGGLE` and `CHECKBOX_GROUP` custom fields uses `!value || !value.trim()`~~ **Fixed (2026-04-05):** TOGGLE now checks `value !== "true"`; CHECKBOX_GROUP splits on comma, filters empty strings, then checks length. Both cases correctly enforce required status. | ~~Bug~~ |
...
*Updated: 2026-04-05 — Closed Known Issues #1 and #3; updated Known Issue #10 (action.ts also uses bare "zod"); user table: added firstName/lastName (PR #77); Production: added description/banner (PR #62); Role: added referencePhotos (PR #62); Submission: removed denormalized candidate fields (PR #75), added sortOrder/videoUrl/unionStatus/representation/resumeText; File: added formFieldId column and CUSTOM_FIELD enum value (PR #63); updated SystemFieldConfig shape; email.ts: added sendBatchEmail() (PR #78)*
