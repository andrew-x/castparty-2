...
| 6 | `src/actions/submissions/create-submission.ts` | `createSubmission` is not wrapped in a transaction. The `Candidate` upsert, `Submission` insert, `File` inserts, and resume-text update are separate statements. A crash mid-way leaves partial state. **TODO comment added** — blocked on switching from `neon-http` to `neon-serverless` driver. | Data integrity |
| 7 | `src/actions/productions/reorder-role-stages.ts` | Stage reorder uses `Promise.all` with individual `UPDATE` statements — non-atomic. A partial failure leaves stages in an inconsistent order. Same pattern in `reorder-production-stages.ts`. **TODO comment added** — same driver blocker as #6. | Data integrity |
