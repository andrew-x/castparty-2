...
> **Status:** Active development — auth & onboarding, organizations, productions, candidates, public submission flow, configurable pipeline stages, custom form builders (4 contexts: production/role × submission/feedback), reject reasons, role Kanban board with drag-and-drop / bulk actions / comparison view / search / compact view, stage browse view, submission detail sheet, headshot lightbox, candidate detail, comments on submissions, admin, file uploads (headshots + resume with PDF text extraction), feedback (per-stage, with ratings and custom fields), and dev-only email emulator are shipped. Known issues tracked in `docs/ARCHITECTURE.md#known-issues`.
